...
import java.util.{ArrayList => JList, Map => JMap}
...
class CHMergeTreeWriterInjects extends GlutenFormatWriterInjectsBase {
  ...
  override def createOutputWriter(
    ...
    val uuid = context.getConfiguration.get("mapreduce.mergetree.job.uuid")
    ...
    val writeInfoPlan = CHMergeTreeWriterInjects.genMergeTreeWriteRel(
      ...
      partitionColumns,
      bucketSpec,
      MergeTreePartsPartitionsUtil.genNamedStructJson(tableSchema),
      dataSchema
    )
...
object CHMergeTreeWriterInjects {
  ...
  def genMergeTreeWriteRel(
    ...
    val (orderByKey, primaryKey) = MergeTreePartsPartitionsUtil.genOrderByAndPrimaryKeyStr(
      orderByKeyOption,
      primaryKeyOption
    )
