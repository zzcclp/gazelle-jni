  ...
  protected def releaseCurrentWriter(): Unit = {
    if (currentWriter != null) {
      try {
        ...
        val ret = currentWriter.asInstanceOf[MergeTreeOutputWriter].getAddFile()
        if (ret.isDefined) {
          returnedMetrics.put(ret.get.path, ret.get)
        }
      } finally {
        currentWriter = null
      }
      ...
      status => {
        if (status.outputWriter != null) {
          try {
            ...
            val ret = status.outputWriter.asInstanceOf[MergeTreeOutputWriter].getAddFile()
            if (ret.isDefined) {
              returnedMetrics.put(ret.get.path, ret.get)
            }
          } finally {
