...
import org.apache.spark.sql.execution.adaptive.{AdaptiveSparkPlanExec, AdaptiveSparkPlanHelper, ColumnarCustomShuffleReaderExec, CustomShuffleReaderExec}
  ...
  test("TPCH Q1") {
    runTPCHQuery(1) { df =>
      ...
      val customShuffleReaderExecs = collect(df.queryExecution.executedPlan) {
        case csr: CustomShuffleReaderExec => csr
      }
      ...
      val colCustomShuffleReaderExecs = collect(df.queryExecution.executedPlan) {
        case csr: ColumnarCustomShuffleReaderExec => csr
      }
