...
object TimestampAddImpl {
  ...
  def evaluateDays(unit: String, increment: Int, time: Int): Int = {
    ...
    ExtendDateTimeUtils.millisToDaysLegacy(
  ...
  def evaluateDays(unit: String, increment: Long, time: Int): Int = {
    if (increment > Int.MaxValue) {
      throw new IllegalArgumentException(s"Increment($increment) is greater than Int.MaxValue")
    }
  ...
  private def addTime(unit: String, increment: Int, cal: Calendar): Unit = {
    unit.toUpperCase(Locale.ROOT) match {
      ...
      /* case "MONTH" | "SQL_TSI_MONTH" =>
        cal.setTimeInMillis(ExtendDateTimeUtils.addMonths(cal.getTimeInMillis * 1000, increment) / 1000)
      case "QUARTER" | "SQL_TSI_QUARTER" =>
        cal.setTimeInMillis(ExtendDateTimeUtils.addMonths(cal.getTimeInMillis * 1000, increment *
         MONTHS_PER_QUARTER.intValue()) / 1000)
       */
