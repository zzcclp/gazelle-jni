...
object TimestampDiffImpl {
  ...
  def evaluate(unit: String, timestamp: Long, date: Int): Long = {
    ...
    val after = ExtendDateTimeUtils.daysToMillis(date)
  ...
  def evaluate(unit: String, date1: Int, date2: Int): Long = {
    val before = ExtendDateTimeUtils.daysToMillis(date1)
    val after = ExtendDateTimeUtils.daysToMillis(date2)
    convertDuration(unit, before, after)
  }

  // DateType -> TimestampType
  def evaluate(unit: String, date: Int, timestamp: Long): Long = {
    val before = ExtendDateTimeUtils.daysToMillis(date)
  ...
  private def convertDuration(unit: String, bMillis: Long, aMillis: Long): Long = {
    unit.toUpperCase(Locale.ROOT) match {
      ...
      /* case "MONTH" | "SQL_TSI_MONTH" =>
        ExtendDateTimeUtils.subtractMonths(aMillis, bMillis)
      case "QUARTER" | "SQL_TSI_QUARTER" =>
        ExtendDateTimeUtils
          .subtractMonths(aMillis, bMillis) / ExtendDateTimeUtils.MONTHS_PER_QUARTER
      case "YEAR" | "SQL_TSI_YEAR" =>
        ExtendDateTimeUtils
          .subtractMonths(aMillis, bMillis) / ExtendDateTimeUtils.MONTHS_PER_QUARTER /
          ExtendDateTimeUtils.QUARTERS_PER_YEAR
       */
      case _ =>
